...
PRESONAL_INFO_VALUE_DATA_TAG = "DataText"
INFO_HEADER_TITLE = "Title2"
...
class MKHtmlParser(object):
	...
	def _parse_mk_info_data(self):
		'''The function parse the mk information and returns dict with all the extracted info'''
		info_dict = {}
	...
	def __is_presonal_info_tag(self,tag):
		...
		if (tag['class'] != PRESONAL_INFO_KEY_DATA_TAG) and (tag['class'] != PRESONAL_INFO_VALUE_DATA_TAG):
			return False						
		return True
	...
	def _get_general_personal(self, tr_list):
		...
		for tr in tr_list:			
			...
			k,v = (None,None)
			for td in tds:						
					if td['class'] == PRESONAL_INFO_KEY_DATA_TAG:
						k = td.string
						if (k is None or k == '') and len(td.findAll('a')) > 0 :
							k = td.findAll('a')[0].string
					if td['class'] == PRESONAL_INFO_VALUE_DATA_TAG:
						v = td.string
