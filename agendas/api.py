    ...
    def dehydrate_members(self, bundle):
        ...
        for mk in Member.objects.filter(pk__in=mks_values.keys(),
            ...
            members.append(dict(
                id=mk.id,
                name=mk.name,
                score=map(itemgetter('score'),mk_data) if len(mk_data)>1 else mk_data[0]['score'],
                rank=map(itemgetter('rank'),mk_data) if len(mk_data)>1 else mk_data[0]['rank'],
                volume=map(itemgetter('volume'),mk_data) if len(mk_data)>1 else mk_data[0]['volume'],
                numvotes=map(itemgetter('numvotes'),mk_data) if len(mk_data)>1 else mk_data[0]['numvotes'],
