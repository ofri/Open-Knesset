...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        """
        go over all the corporation data and remove any which don't include any changes in them
        """
        corp_lobbyist_ids = {}
        for corpdata in orm.LobbyistCorporationData.objects.order_by('scrape_time'):
            lobbyist_ids = map(lambda lobbyist: lobbyist.id, sorted(corpdata.lobbyists.all(), key=lambda lobbyist: lobbyist.id))
            if corpdata.corporation.id in corp_lobbyist_ids and lobbyist_ids == corp_lobbyist_ids[corpdata.corporation.id]:
                # it's not the first occurence of this corporation and the lobbyist ids match this data's lobbyist ids
                # we can delete this corporation data object
                corpdata.delete()
            else:
                # this is either the first occurence of this corporation
                # or it's a corporation data which doesn't contain any changes (the same lobbyists here and in previous one)
                # we don't delete it, just store it internally for reference with next data
                corp_lobbyist_ids[corpdata.corporation.id] = lobbyist_ids
